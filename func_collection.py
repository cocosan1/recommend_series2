...
def pre_processing(df_now, df_last, selected_base, selected_cate):
    if selected_cate == 'リビングチェア':
        ...
        df_now2.loc[:, '品番'] = df_now2['商　品　名'].apply(lambda x: x.split(' ')[0])
        df_last2.loc[:, '品番'] = df_last2['商　品　名'].apply(lambda x: x.split(' ')[0])

        df_now2.loc[:, '商品コード2'] = df_now2['商品コード'].apply(lambda x: x.split()[0]) #品番
        df_last2.loc[:, '商品コード2'] = df_last2['商品コード'].apply(lambda x: x.split()[0]) 

        df_now2.loc[:, selected_base] = df_now2[selected_base].fillna(0)
        df_last2.loc[:, selected_base] = df_last2[selected_base].fillna(0)

        df_now2.loc[:, '受注年月'] = df_now2['受注日'].dt.strftime("%Y-%m")
        df_now2.loc[:, '受注年月'] = pd.to_datetime(df_now2['受注年月'])

        df_last2.loc[:, '受注年月'] = df_last2['受注日'].dt.strftime("%Y-%m")
        df_last2.loc[:, '受注年月'] = pd.to_datetime(df_last2['受注年月'])
